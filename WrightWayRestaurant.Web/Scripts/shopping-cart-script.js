...
function ShoppingCarObserver(elInput, isAdd) {
	...
	this.computeGoodsMoney = function() {
	    var moneyCount = parseFloat(this.count) * parseFloat(this.singlePrice);
	    var singleTotalEl = this.parents.find('.single-total');
	    console.log(moneyCount);
	    singleTotalEl.empty().append(parseFloat(moneyCount).toFixed(2));
	}
	...
	this.checkedChange = function(isChecked) {
		...
		var goodsTotalCount = parseInt($('#selectGoodsCount').text())
		if(isChecked) {
		    $('#selectGoodsMoney').empty().append(parseFloat(itemTotalMoney + GoodsTotalMoney).toFixed(2))
